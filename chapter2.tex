...
Ideas from this theme give us that our long exact sequence is infact a \textit{cyclic} exact sequence of length $6$:
...
\begin{definition}
Let $A$ be a $C^{*}$-algebra and let $\rho: G \rightarrow Aut(A)$ be a representation of a group $G$. Then we can naturally form the $C^{*}$-algebra $A\rtimes_{r}G$; it is the completion of...
\end{definition}
...
The main idea of this result is that it gives an understanding of a crossed product structure by looking at only the induced action on the K-theory groups of the coefficient algebra. The main issue is that in general these can be as bad as one wants, so computations of the action could be particularly bad.
