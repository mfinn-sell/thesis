...
\item $p$ is said to be \textit{homotopic} to $q$, $p \sim_{h} q$, is there exists a norm continuous path of projections $p_{t}$ such that $p_{0}=p$ and $p_{1}=q$.
\end{enumerate}
\end{definition}
...
To construct the K-theory group we need a construction that associates to every commutative monoid a commutative group. We do this as follows. Let $M$ be a commutative monoid with operation $+$. Now consider the following equivalence relation on $M\times M$:
\begin{equation*}
(s,t) \tilde (u,v) \Leftrightarrow (\exists p \in M) \mbox{ such that } s+v+p=u+t+p
\end{equation*} 
The quotient of $M\times M$ by this relation has a group structure extending the $+$ on $M$ \cite{MR1222415}. This also has a natural injective map from $M$, given by sending $m\in M$ to $[m,1]_{\tilde}$. This process generalises the construction of the integers from the natural numbers and is called taking the \textit{Grothendieck group} of $M$.
...
The definition of $K_{1}$ is more technical and is constructed from unitaries instead of projections. We present it tersely here. Let $X$ be a topological space. We denote by $X_{0}$ the set of connected components of $X$.
