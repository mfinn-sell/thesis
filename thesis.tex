...
In this section we outline the coarse geometry and group theoretic properties that are going to be considered throughout this document. The overall scheme of this section is first to consider some general coarse ideas, such as coarse structures, and then move onto discussions of certain analytic properties held by discrete groups.

\subsection{Coarse geometry.}
Some notions of coarse information need to go here, namely coarse equivalence, coarse structure etc. 
...
In this chapter we prove the following result concerning the $C^{*}$-algebras associated to an F-inverse monoid.

\begin{theorem}\label{thm:C2T1}
Let $S$ be an F-inverse monoid and let $A=C_{c}(\G_{U})$. Then we have the following (intrinsic) short exact sequence of $C^{*}$-algebras:
\begin{equation}\label{eqn1}
0 \rightarrow \overline{A} \rightarrow C^{*}_{r}(\mathcal{G}) \rightarrow C^{*}_{r}(G) \rightarrow 0
\end{equation}
\end{theorem}

These ideas are then refined to F-inverse monoids associated to certain metric spaces.

\subsection{Machinery for Theorem \ref{thm:C2T1}}
...
\end{theorem}
\begin{proof}
The Haagerup property provides us with the Strong Baum-Connes property for even the maximal group conjecture, boiling down to the fact that such groups are K-amenable even with coefficients. It then follows from Proposition \ref{Prop:Max}.
\end{proof}
...
In this section we outline the construction of a space for which the boundary Baum-Connes conjecture fails to be surjective. This construction first appeared in \cite{MR2363697}.
...
On the other hand, the question of when a groupoid admits a transformation groupoid decompostion, up to Morita equivalence, has been well studied for the class of groupoids that are constructed from suitable inverse semigroups \cite{MR1900993,Milan-Steinberg} that admit a \textit{cocycle} onto a group, or groupoid:
...
\section{Groupoid Valued Cocycles.}
...
\subsection{Partial Translation Structures.}\label{Sect:PTS}

Recall Definition \ref{Def:PTS}
\begin{definition}
Let $X$ be a countable discrete metric space. A collection of partial bijections $\mathcal{T}$ is a called a partial translation structure for $X$ if:
\begin{enumerate}
\item $\mathcal{T}$ partitions $X\times X$.
\item $\forall t_{i}, t_{j} \in \mathcal{T}$ $\exists t_{k} \in \mathcal{T}$ we have $t_{i}t_{j} \subseteq t_{k}$ (i.e $\mathcal{T}$ is subclosed).
\item $\forall t \in \mathcal{T}$ we have $t^{*} \in \mathcal{T}$.
...
The precise nature of the relationship between partial translation structures in the sense of definition \ref{Def:PTS} and uniform embeddings is understood. It follows from \cite[Theorem 19]{MR2363428} that given any space that admits a uniform embedding into a group, we can equip it with a translation structure given by the definition \ref{Def:PTS}. The inverse monoid generated by this translation structure is also understood; recall \cite[Proposition 3.37]{mypub1}: 

\begin{proposition}\label{Prop:3.37}
Let $S$ be the inverse monoid generated by a dual prehomomorphism $\theta: G \rightarrow I(X)$. If $S$ is $0$-F-inverse with maximal element set $\lbrace \theta_{g} | g \in G \rbrace$ and no non-zero $\theta_{g}$ is an idempotent then $S$ is strongly $0$-F-inverse.\qed
\end{proposition}

This has an immediate conseqeunce for the inverse monoid generated by a partial translation structure arising from a truncation of a group action to a coarsely embedded space.

\begin{corollary}\label{Cor:Trick}
Let $X$ be coarsely embedded into $G$. Then the translation structure $\mathcal{T}$ arising from truncation of the action generates a strongly $0$-F-inverse monoid. 
\end{corollary}
\begin{proof}
We remark that as $G$ acts on itself freely, the truncation has the property that no $t_{g}$ that is non-zero will be idempotent. The result now follows from Lemma \ref{Lem:PTS} and Proposition \ref{Prop:3.37}.
\end{proof}

\begin{proposition}
Let $\mathcal{T}$ be a partial translation structure on a space $X$, and let $S:= S(\mathcal{T})$. Then $S$ is a strongly 0-E-unitary inverse monoid then we can build a locally compact Hausdorff \'etale groupoid $\mathcal{G}(S)$, and a representation $\pi_{X}$ (and a reduction $\G(\mathcal{T})$) of $\G(S)$ on $X$. 
\end{proposition}
\begin{proof}
The first half of this follows from section \ref{Sect:StoG}.

To build the groupoid $\G(\mathcal{T})$ we first build a space $\widehat{X}$, which is done by considering the translation structure and the inverse monoid it generates inside $\mathcal{I}(X)$. Then we use this space to construct a reducuction the universal groupoid $\G_{\E(S)}$. 

We build the unit space $\widehat{X}$ of the groupoid using some results from \cite[Proposition 10.6, Theorem 10.16]{MR2419901}. The first leads us to consider the representation of the inverse monoid $S$ on $\ell^{2}(X)$ induced by...  to get a representation $\pi_{\theta}:S \rightarrow \mathcal{B}(\ell^{2}(X))$. We can complete the semigroup ring in this representation to get an algebra $C^{*}_{\pi_{\theta}}S$, which has a unital commutative subalgebra $C^{*}_{\pi_{\theta}}E$. \cite[Proposition 10.6]{MR2419901} then tells us that the spectrum of this algebra, which we will denote by $\widehat{X}$, is a subspace of $\E$ that is closed and invariant under the action of $S$ on which the representation $\pi_{\theta}$ is supported (see \cite[Section 10]{MR2419901} for more details). 

As the space $\widehat{X}$ is closed and invariant we can reduce the universal groupoid $\G_{\E}$ for $S_{inf}$ to $\widehat{X}$. This we denote by $\G(\mathcal{T})$. \cite[Theorem 10.16]{MR2419901} implies that with this representation in hand we have the isomorphism: $C^{*}_{r}(\G(\mathcal{T})) \cong C^{*}_{\pi_{\theta}}(S)\cong C^{*}\mathcal{T}$.\end{proof}
...
\subsection{The original construction.}

Fix a left invariant proper metric on $G$.
...
\subsection{The proof of Theorem \ref{Thm:MT1}.}

The procedure we will follow will be a geometric analogue of this argument using translation structures. Recall the following definition from \cite{MR1911663}:
...
\begin{proof}(Of Theorem \ref{Thm:MT1})
We equip $X$ with the truncation partial translation structure coming from its coarse embedding into $G$. Using Lemma \ref{Lem:CG} and Proposition \ref{Prop:Cocycle}, we have the coarse groupoid $G(X)$ is isomorphic to $\beta X \rtimes \G(\mathcal{T})$ and that this admits a (T,C,F)-cocycle back onto $G$. From this cocycle we construct a locally compact Hausdorff space $Y_{2}$ by taking $\beta X \times G$ and modding out by $\sim$. It then follows from either \cite[Theorem ]{Milan-Steinberg} (Theorem \ref{}) or \cite[Theorem]{MR1900993} (Theorem \ref{Thm:1.8}) that $G(X) \cong \beta X \rtimes \G(\mathcal{T})$ is Morita equivalent to $Y_{2}\rtimes G$. 
...
\begin{equation*}
\xymatrix@=1em{...\ar[r] & K_{0}(C_{0}(U)\rtimes \Gamma) \ar[r]& K_{0}(C_{0}(Z)\rtimes \Gamma) \ar[r]& K_{0}(C_{0}(Y)\rtimes \Gamma)\ar[r] & ...\\
...
\section{Boundary Coefficients for a Gromov Monster.}
