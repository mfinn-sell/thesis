...
\item For any continous vector field $h(x)$ in $H$ and every $g \in \Gamma$ we have that $A_{(x,g)}(h(xg))$ is a continous vector field in $H$.
\end{enumerate}
\end{definition}
...
\end{conjecture}
This conjecture is slightly naive, as it is not in fact functorial under coarse maps. The left hand side, $K_{*}(X)$ is a topological homology theory, functorial under proper continous maps (see Chapter 5 of \cite{MR1399087}), whilst the right hand side is a coarse invariant. Thus, the class of spaces that naturally fit this conjecture are those whose topology is uniformly controlled, that is \textit{uniformly contractible spaces}
...
Given a $\underline{E}X$ we could then apply Conjecture \ref{conj:CBC1} directly and use coarse invariance of the right side. The issue is that these spaces need not exist in general and so we must work around this problem by constructing a weaker notion that does always exist. 
...
\end{equation*}
By taking a direct limits we get the following:
\begin{equation*}
...
To see (2) we use Lemma \ref{Lem:Lemma1} part 1 or directly: Take any non-compact ghost projection $p \in K_{0}(I_{G})$. This does not lie in the image of $\mathbb{Z}$ as it has non-zero image under $d_{*}$. Push this element to $q \in K_{0}(C^{*}_{r}(G(X))$. Assume for a contradiction that $\mu$ is surjective. Then there is an element $x \in \mathbb{Z}$ that maps to $q$, and so $d_{*}(q)=0$, as the image of compact operators lies in the kernel of the map $d_{*}$. However, we know that $d_{*}(q)=d_{*}(p)$ is certainly non-zero.
\end{proof}
