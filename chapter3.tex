...
The map that sends $s \in S$ to $s \in U(S)$ is a partial homomorphism that is universal for partial homomorphims onto groups. It also induces a prehomomorphism $S \rightarrow U(S)^{0}$. An inverse monoid $S$ is strongly $0$-E-unitary if and only if this prehomomorphism is idempotent pure.
...
Let $X$ be a countable set. A collection of partial bijections $\mathcal{T}$ is a called a \textit{abtract partial translation structure} for $X$ if:
\begin{enumerate}
...
Condition (2) ensures that such a structure is compatable with the metric.
...
\end{equation}
where $x_{0}$ is a basepoint in $X$. Observe that because $\mathcal{T}$ partitions $X \times X$ we can construct a bijection between $X$ and $T_{x_{0}}$, specifically $x \mapsto t_{x}$, where $t_{x}$ is the element of $T_{x_{0}}$ satisfting $t_{x}(x)=x_{0}$. Restricting to the image of $T_{x_{0}}$ under $\sigma$ we get a subspace of the group that is in bijection with $X$, i.e we can view $X$ as a subset of the group $G$. To finish the proof, we need the translation structure $\mathcal{T}$ to come from the group.
...
Additionally, we will be interested in describing the elements of $\X$ in terms of filters and ultrafilters of $X$. We restrict now to the case that $\mu$ is a bounded represenation and $S$ is a monoid. In this instance $A \subset \ell^{\infty}(X)$, and by considering spectra we get a natural map: $\beta X \twoheadrightarrow \X$.
...
It is immediate (using \cite[Prop 10.10]{MR2419901}) that the set $\X$ is invariant under the action of $S$. To compute the groupoid and groupoid $C^{*}$-algebras associated to $\X$ we would like to know a little more about the Hilbert spaces associated to the fibers and general connectness of the set $B:=\lbrace \psi_{x} | x \in X \rbrace$, which is the image of $X$ in $\X$.
...
$U_{x}$ impliments a spacial equivalence of norms, that is: $\Vert \lambda(1_{tt^{*}}\delta_{t}) \Vert = \Vert \mu(t) \Vert_{\ell^{2}(X)}$ for all $t \in \mathcal{T}$.
\end{propositon}
\begin{proof}
