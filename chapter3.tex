...
These definitions allow for a rephrasing to the language of inverse semigroups of the definitions in \cite{MR2363428}. 
...
Recall that a subspace $Y$ of a metric space $X$ is said to be \textit{coarsely dense} if there exists $c>0$ such that for every $x \in X$ there is a $y \in Y$ satisfying $d(x,y) < c$.

\begin{proposition}\label{prop:TFAE} Let $G$ be a countable discrete group with a proper left invariant metric and let $X \subseteq G$ be a metric subspace.
The following are equivalent:
\begin{enumerate}
\item $X^{c}$ is not coarsely dense in $G$.
\item For every $R>0$ there exists $g \in G$ such that $B_{R}(g) \subseteq X$.
\item The monoid generated by the translations obtained by restriction of the right action of $G$ on itself has no zero element.
\end{enumerate}
\end{proposition}
...
Let $X$ be a countable set. A collection of partial bijections $\mathcal{T}$ is a called an \textit{abstract partial translation structure} for $X$ if:
\begin{enumerate}
\item $\mathcal{T}$ partitions $X\times X$.
\item $\forall t_{i}, t_{j} \in \mathcal{T}$ $\exists t_{k} \in \mathcal{T}$ such that $t_{i}t_{j} \subseteq t_{k}$ (i.e $\mathcal{T}$ is subclosed).
...
\begin{definition}
A (grouplike) partial translation structure on a metric space $X$ is an abstract partial translation structure $\mathcal{T}$ where:
\begin{enumerate}
...
\begin{proof}
Consider the inverse monoid $S = \langle \mathcal{T} \rangle$. By Lemma \ref{Lem:PTS} $S$ is $0$-F-inverse, and as $\mathcal{T}$ has no zero divisors we know that in fact it must be F-inverse. As every $t \in \mathcal{T}$, $t\not =1$ is not an idempotent, we can conclude that that $S/\sigma$ is a non-trivial group. Denote that group by $G$. The aim now is to embed $X$ into $G$. The maximal elements in $\mathcal{T}$ generate this group, and $\sigma$ induces an inverse semigroup homomorphism from $S$ into $G$, which is a bijection between the maximal elements and $G$. Denote by $T_{x_{0}}$ the following:
\begin{equation}
...
\begin{definition}
The translation algebra associated with a partial translation structure $\mathcal{T}$ on $X$, denoted by $C^{*}\mathcal{T}$, is the $C^{*}$-algebrea generated by $\mathcal{T}$, when each translation is viewed as bounded operator on $\ell^{2}(X)$
\end{definition}
...
\begin{eqnarray*}
j& : & \widehat{X} \rightarrow \E \\
& &\psi \mapsto  \phi=\psi \circ \mu
\end{eqnarray*}
...
\end{itemize}
It is impossible for both $M$ and $M^{c}$ to be contained in $F_{\psi}$ as $E(S)$ has no zero element.
...
In the case that $S$ has a zero element, we appeal to universal properties and another result of Exel \cite{MR2419901}. First we need a definition, making use of the groupoid $G(X)$ defined in Chapter 2.
\begin{definition}
Let $G(X)$ be the coarse groupoid of $X$ as defined in Chapter 2. Then the \textit{uniform Roe algebra}, denoted by $C^{*}_{u}(X)$, is the reduced groupoid $C^{*}$-algebra $C^{*}_{r}(G(X))$.
\end{definition} 

We now finish the proof of Proposition \ref{prop:P9}. By Proposition 10.10 \cite{MR2419901} the space $\widehat{X}$ is closed and invariant.

Recall that since $t \in \mathcal{T}$ is an element of $I_{b}(X)$ the algebra $C^{*}_{\pi}(S)$ is a subalgebra of the uniform Roe algebra $C^{*}_{u}X$. We now remark that the representation $\pi_{X}$, when restricted to $C^{*}E$ assigns each idempotent a projection in $C^{*}_{u}X$, that is $C^{*}_{\pi_{\mu}}(E)=\pi_{}(C^{*}E) \subset \ell^{\infty}(X)$. Taking the spectra associated to this inclusion then gives us a map:
\begin{equation*}
...
This lets us define a map on the basis of $C_{c}(\G_{\widehat{X}})$, which when extended linearly has image $\mu(\mathbb{C}S)$. This map is given by: 
\begin{equation*}
...
Let $X$ be a subset of a countable discrete group $G$ equipped with the natural partial translation structure $\mathcal{T}_{G}|_{X}$v arising from a proper left invariant metric.  Let $\mu$ denote the representation of $S = \langle \mathcal{T}_{G}|_{X} \rangle$ on $X$. If $\mathcal{T}_{G}|_{X}$ has no zero divisors then we have an isomorphism C^{*}_{r}(\G_{\X}) \cong C^{*}_{\mu}(S) = C^{*}\mathcal{T}. 
\end{theorem}
